...
namespace RogueEssence.Dev.ViewModels
{
    public class DevTabConstantsViewModel : ViewModelBase
    {
        ...
        public DevTabConstantsViewModel(EditorContext context)
        {
            _context = context;
        }
        ...
        public async void mnuUniversalFile_Click()
        {
            ...
            {
                await MessageBoxWindowView.Show(_context.DialogService, "Universal data must have saved edits first!", "Error", MessageBoxWindowView.MessageBoxButtons.Ok);
                return;
            }
            ...
            await MessageBoxWindowView.Show(_context.DialogService, "Universal is now saved as a file.", "Complete", MessageBoxWindowView.MessageBoxButtons.Ok);
        }
        public async void mnuUniversalDiff_Click()
        {
            ...
            if (DataManager.GetDataModStatus(DataManager.DATA_PATH, "Universal", DataManager.DATA_EXT) == DataManager.ModStatus.Base)
                await MessageBoxWindowView.Show(_context.DialogService, "Modded Universal was identical to base. Unneeded patch removed.", "Complete", MessageBoxWindowView.MessageBoxButtons.Ok);
            else
                await MessageBoxWindowView.Show(_context.DialogService, "Universal is now saved as a patch.", "Complete", MessageBoxWindowView.MessageBoxButtons.Ok);
        }
        ...
        private void OpenItem<T>(string name, T data, Action<T> saveOp, NodeBase parentNode)
        {
            lock (GameBase.lockObj)
            {
                ReflectedDataPageViewModel newEditor = _context.PageFactory.CreatePage<ReflectedDataPageViewModel>(parentNode);
                newEditor.SetPageTitle(name, parentNode.Icon);

                newEditor.OnLoadAction = (StackPanel stack) =>
                {
                    DataEditor.LoadDataControls("", data, stack);
                };
