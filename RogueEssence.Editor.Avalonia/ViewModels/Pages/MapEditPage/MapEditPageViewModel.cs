...
public class MapEditPageViewModel : EditorPageViewModel, IMapEditor, IPreCreatePage, IPageReloadable
{
    ...
    {
        DevForm.ExecuteOrInvoke(() => _processInput(input));
    }
    
    public MapEditPageViewModel(
        ...
        EditorContext context, NodeBase node, Action<EditorPageViewModel> onPageOpen = null) : base(context, node,
        onPageOpen)
    {
        Edits = new UndoStack();
    }
    ...
    public async void mnuExportAsGround_Click()
    {
        ...
        if (!String.IsNullOrEmpty(result))
        {
            ...
            if (!comparePaths(reqDir, Path.GetDirectoryName(result)))
                await MessageBoxWindowView.Show(_context.DialogService, String.Format("Map can only be saved to:\n{0}", reqDir),
            ...
            else
            {
                ...
                await MessageBoxWindowView.Show(_context.DialogService, String.Format("Textures have been saved to Ground!"), "Success",
                    MessageBoxWindowView.MessageBoxButtons.Ok);
            }
        }
    }
    ...
    public void LoadFromCurrentMap()
    {
        if (ZoneManager.Instance.CurrentMap.AssetName != "")
            CurrentFile = PathMod.ModPath(Path.Combine(DataManager.MAP_PATH,
                ZoneManager.Instance.CurrentMap.AssetName + DataManager.MAP_EXT));
        else
            CurrentFile = "";
    ...
    private void DoImportTileset(string sheetName)
    {
        ...
        for (int yy = 0; yy < newSize.Y; yy++)
        {
            for (int xx = 0; xx < newSize.X; xx++)
                ZoneManager.Instance.CurrentMap.Layers[Textures.Layers.ChosenLayer].Tiles[xx][yy] =
                    new AutoTile(new TileLayer(new Loc(xx, yy), sheetName));
        }
    ...
    private void ExportToGround(Map curmap, string filepath)
    {
        ...
        GroundEditPageViewModel.CreateOrCopyScriptData("", filepath);
    ...
    public void _processInput(InputManager input)
    {
        lock (GameBase.lockObj)
        {
